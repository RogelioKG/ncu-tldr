...
async def test_delete_wish(client: AsyncClient) -> None:
    ...
    deleted = await client.delete(f"/api/wishlist/{wish_id}")
    assert deleted.status_code == 204


@pytest.mark.asyncio
async def test_delete_missing_wish(client: AsyncClient) -> None:
    response = await client.delete("/api/wishlist/9999")
    assert response.status_code == 404


@pytest.mark.asyncio
async def test_wishlist_sorted_by_vote_count(client: AsyncClient) -> None:
    await client.post("/api/wishlist", json={"name": "A課程", "teacher": "A老師"})
    await client.post("/api/wishlist", json={"name": "A課程", "teacher": "A老師"})
    await client.post("/api/wishlist", json={"name": "B課程", "teacher": "B老師"})
