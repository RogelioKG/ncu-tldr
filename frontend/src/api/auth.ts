...
const mockUsers = new Map<string, { id: string, password: string, displayName: string }>([
...
export async function register(payload: RegisterPayload): Promise<AuthResult> {
  ...
  const nextId = String(Math.max(...Array.from(mockUsers.values(), row => Number(row.id)), 0) + 1)
